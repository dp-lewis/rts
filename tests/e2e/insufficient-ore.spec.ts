...
test.describe('EDGE-004 — cannot afford it', () => {
  test('the unaffordable entry is greyed but still shows its cost', async ({ page }) => {
    ...
    // Starting ore is 150; a Tank costs 160 and a Factory 200.
    const tank = page.getByTestId('build-tank');
    await expect(tank).toBeVisible();
    await expect(tank).toHaveAttribute('aria-disabled', 'true');
    await expect(tank).toContainText('160');
  });

  test('nothing is hidden — all five entries stay on screen', async ({ page }) => {
    await openGame(page);
    await startMatch(page, 'easy');
    await expect(page.getByTestId('build-bar').locator('[data-testid^=build-]')).toHaveCount(5);
  });
  ...
  test('clicking it opens no dialog and spends nothing', async ({ page }) => {
    ...
    await page.getByTestId('build-tank').click({ force: true });
  ...
  test('an affordable entry is not greyed, so the state means something', async ({ page }) => {
    ...
    await openGame(page);
    await startMatch(page, 'easy');
    await expect(page.getByTestId('build-worker')).not.toHaveAttribute('aria-disabled', 'true');
  });
  ...
  test('the greyed state clears once the ore arrives', async ({ page }) => {
    ...
    await expect(page.getByTestId('build-tank')).not.toHaveAttribute('aria-disabled', 'true');
  });
});
